...
function manifest(violations: readonly string[] = []): EgressManifest {
  return {
    ...
    // EGR-01 — π submits every shard public, so the run this fixture stands for registered
    // nothing. Stated rather than omitted: the omission would select a different arm of
    // `egressLines`, and the arm it selects is the one that used to be false.
