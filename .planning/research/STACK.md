...
**Domain:** TypeScript P2P compute fabric (browser + Node.js + embedded), WASM execution, content addressing
**Researched:** 2026-07-24
**Confidence:** HIGH for versions and libp2p/Helia topology (verified against npm registry + `main` branch source on 2026-07-24). MEDIUM for WASM determinism strategy (verified primitives, unverified end-to-end). MEDIUM for Safari-specific WebRTC behaviour.
...
1. **js-libp2p is at `3.3.6`, not 2.x.** v3 is a hard API break: streams are now `EventTarget`s (`.send()` / `message` event), not streaming-iterables (`.source` / `.sink`). Protocol handlers changed signature. Any code, tutorial, or LLM output written for libp2p 1.x/2.x **will not compile**. [Migration guide](https://github.com/libp2p/js-libp2p/blob/main/doc/migrations/v2.0.0-v3.0.0.md)
...
3. **`@ipshipyard/libp2p-auto-tls` dissolves the WSS-certificate constraint.** A Node relay listening on `/ip4/0.0.0.0/tcp/0/ws` automatically acquires a Let's Encrypt cert for `<peerId>.libp2p.direct`, giving a browser-dialable `DNS + WSS` multiaddr with zero cert ops. This is the single highest-leverage package for the browser tier.
...
5. **TypeScript 7.0.2 shipped (2026-07-08)** — the Go-native compiler. `strict` on by default, `module: esnext` default, `node10`/`classic` resolution removed, `baseUrl` removed, `types` defaults to `[]`. **No stable programmatic API until 7.1** — so pick a toolchain that does not depend on the TS API.
6. **`tsup` is explicitly unmaintained** (its own README says so). Use `tsdown`.
...
The canonical, mutually-compatible set, taken from [`packages/integration-tests/package.json`](https://github.com/libp2p/js-libp2p/blob/main/packages/integration-tests/package.json) on `main` (this is the set libp2p itself CI-tests together — do not assemble your own from tutorials):
...
npm install blockstore-idb@4.0.1 datastore-idb@5.0.1
...
# ---- Crypto / protocol ----
npm install @noble/hashes@2.2.0 @noble/curves@2.2.0 protons-runtime@7.0.0

# ---- WASM guest support ----
npm install @bjorn3/browser_wasi_shim@0.4.2 wasm-feature-detect@1.8.0
...
Verified against `@libp2p/webrtc@6.0.27` and `@libp2p/webtransport@6.0.32` source on `main`, plus [libp2p.io/docs/webrtc-browser-connectivity](https://libp2p.io/docs/webrtc-browser-connectivity/).
...
**Recommendation:** own AutoTLS-backed backbone relays are **primary**; public infra is opportunistic redundancy. Raise `reservations.defaultDurationLimit` / `defaultDataLimit` / `maxReservations` on your own `circuitRelayServer()`. Keep public relays configured so the "works without our infra" claim stays technically true.
...
Browser and Node both expose the same `WebAssembly` API backed by V8. **No third-party WASM runtime belongs in the portable agent.** Wasmtime/WasmEdge appear only in the *build pipeline* (Part I, server-side AOT), never in the node agent.

Use `WebAssembly.compileStreaming(fetch(url))` / `instantiateStreaming` — verified requirements for V8's code cache (design §I.6 already states these; they are correct):
- streaming API only,
- `Content-Type: application/wasm`,
- keyed on the **resource URL** (an IPFS gateway URL over a CID is a perfect immutable key),
- module ≳128 KB.

Pulling bytes through `helia`'s `fs.cat()` and calling non-streaming `WebAssembly.compile` **forfeits code caching**. If you must go through Helia, wrap the async iterable in a `ReadableStream` → `Response` with the right content type, but understand the URL-keyed cache still won't fire (there is no URL). The pragmatic answer: **fetch WASM artifacts by gateway URL, fetch data blocks by Bitswap.**

### Determinism — verified constraints

Sources of WASM nondeterminism, from the [normative spec doc](https://github.com/WebAssembly/design/blob/main/Nondeterminism.md):
...
If guest code needs WASI (elfconv's `aarch64-wasi32` target produces preview1 imports), use **`@bjorn3/browser_wasi_shim@0.4.2` in both browser and Node** — not `node:wasi`. Rationale: it is pure JS with **zero dependencies**, so it runs everywhere, and using the *same* shim on every node means every node has identical host semantics. `node:wasi` is still flagged experimental and its behaviour is not bit-identical to a JS shim. The shim implements a *subset* of preview1 — you will likely fork or wrap it to inject a deterministic clock and PRNG, which is a feature, not a bug.

**Recommendation on elfconv targets:** standardize on `TARGET=aarch64-wasi32` for both browser and Node, and supply WASI from TypeScript. Do **not** use `TARGET=aarch64-wasm INITWASM=1` (the Emscripten browser bundle) — it emits JS glue and an HTML harness, splitting the ABI in two and defeating "one artifact, every target".

---

## Build / packaging: one codebase, three targets

The correct mechanism is **package.json `exports` conditions**, not a bundler flag. This is how libp2p and Helia themselves ship — `@helia/libp2p` has `libp2p-defaults.ts` and `libp2p-defaults.browser.ts` resolved by condition.
...
**tsconfig.json** (TS 7.0):

```jsonc
{
  "compilerOptions": {
    "target": "es2023",
    "module": "esnext",              // TS 7 default anyway
    "moduleResolution": "bundler",   // "node10"/"classic" are REMOVED in TS 7
    "strict": true,                  // TS 7 default; state it explicitly
    "isolatedDeclarations": true,    // ⇐ lets Oxc emit .d.ts without the TS API
    "verbatimModuleSyntax": true,
    "noUncheckedIndexedAccess": true,
    "exactOptionalPropertyTypes": true,
    "types": [],                     // TS 7 default; add "node" only where needed
    "lib": ["es2023", "dom", "webworker"]
  }
}
```

`isolatedDeclarations: true` is the load-bearing choice: it forces explicit return types on exported functions (mild annoyance) but makes `.d.ts` emission a purely syntactic transform, so `rolldown-plugin-dts` uses **Oxc** and nothing in the build pipeline touches TypeScript's unstable 7.0 programmatic API.

---

## Benchmarking detail

The requirement is *"throughput vs node count, reproducibly, in browser and Node."* Three layers:

**1. Transport throughput — use `@libp2p/perf@5.1.9`.**
The [official libp2p perf protocol](https://github.com/libp2p/specs/blob/master/perf/perf.md). `measurePerformance(multiaddr, uploadBytes, downloadBytes)` yields streaming measurements. It defaults to opening a **new connection per run** (so handshake cost is included — the honest number); `reuseExistingConnection: true` isolates steady-state throughput. Cross-implementation, so your numbers are comparable to published go-libp2p/rust-libp2p figures rather than being self-referential.

**2. Fabric throughput vs N — build a harness, but make N cheap.**
...
**These are live traps.** Several are packages that *exist, install cleanly, and look current* but are wrong.
...
**If the node is a backbone server:**
...
**If the node is embedded in a host app (Electron/Tauri/Node service):**
...
**If you need deterministic multi-node tests:**
...
1. **Safari + WebRTC-Direct.** The libp2p docs say WebRTC-Direct works in "most browsers" without enumerating them. WebRTC-Direct relies on SDP munging, historically fragile in WebKit. **Not verified.** Mitigation is already in the stack: Playwright `webkit` instance in CI from day one. If it fails, Safari's browser entry point is WSS-only.
2. **Actual sustained throughput over WebRTC data channels at scale.** No credible published numbers found for js-libp2p WebRTC with N>50 browser peers. `@libp2p/perf` exists precisely to answer this — treat it as a **phase-1 measurement task**, not an assumption.
3. **Whether NaN nondeterminism bites in practice for the target workloads.** The spec permits it; whether x86 vs ARM64 V8 actually diverge on the specific float ops elfconv-lifted code emits is unmeasured. **Cheap experiment, high information value:** run a float-heavy WASM module on an ARM Mac and an x86 Linux box and diff the raw bytes. Do this before designing around it.
4. **Binaryen `denan` on elfconv output.** `denan` converts NaN→0, which is *determinism by mutilation* — it changes program semantics. Whether that is acceptable depends on the workload. Unverified against real elfconv artifacts.
5. **Gas metering.** No maintained JS-side WASM instrumentation tool was found. Either build one on `binaryen@131`, adopt Rust-side `wasm-instrument` in the publish pipeline, or accept Worker-timeout as the only bound. Unresolved.
6. **Public relay availability for `/p2p-circuit` at demo scale.** The 15-reservation default and 128 KiB/2 min limits are verified; how many public relays actually accept anonymous reservations today is not. Treat own-infrastructure as the plan of record.
7. **`typescript-eslint@8.65.0` against TypeScript 7.0.** Not verified. Have `oxlint` as the escape hatch.

---

## Sources

**Primary — npm registry, queried 2026-07-24** (all versions above)
- `npm view <pkg> version | time.modified | deprecated | dependencies`

**Primary — source on `main` branch, read 2026-07-24** (HIGH confidence)
...
**Primary — direct measurement** (HIGH confidence)
...
**Secondary — official docs** (MEDIUM–HIGH confidence)
...
**Tertiary — web search, unverified** (LOW confidence, flagged inline)
...
---
*Stack research for: TypeScript P2P compute fabric (browser + Node.js + embedded), WASM execution, content addressing*
*Researched: 2026-07-24*
