...
| `@libp2p/kad-dht@16.4.0` | `DiscoveryPort` on the backbone | **NOT INSTALLED — corrected 2026-07-28.** No DHT exists in this repository; discovery runs over the relay reservation store (`net/src/rendezvous.ts`). The note that `clientMode: false` "requires public addrs" was used to conclude a browser can never serve records — false: `browser-node.ts:197` listens on `['/p2p-circuit','/webrtc']` and a reserved browser is dialable. See `CLAUDE.md` §DHT reality check. |
