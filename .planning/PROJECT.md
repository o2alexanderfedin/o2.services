...
| **Autonomous execution stops at the Cloudflare boundary** | Owner ruling 2026-08-25. Everything in Phase 29 that needs no account proceeds unattended: the package, the `interface-datastore` binding, the four guards, and the build test that settles open question 1. **The billing alert and the first deploy are owner acts.** Phase 29's criteria 1 and 2 therefore stay open and must be *reported* open — ticking either on locally-done work would widen what counts as passing | — Standing **AMENDED 2026-08-30 — the spending alert is configured, owner-stated, and the ruling's protective reason is spent.** What the ruling was guarding is a real hazard with a measured shape: Cloudflare has no hard ceiling, its own wording for budget alerts being *"informational only. It does not cap your usage"*, and the owner's stated budget is $15/month against a measured ~$5/month for one always-on object. With the alert in place the owner now sees an overrun; without it an unattended deploy could have run up a bill nobody was watching. So **deploy is no longer categorically an owner act** — what remains owner-gated is anything that is irreversible OUTWARD (an invite to the tester cohort, Phase 39) or that provisions new billable objects in new regions (Phase 33), because an alert reports a cost, it does not prevent one.
| **HOST-10 stays `Refuted`, and this is the distinction that matters** | The alert existing today does not satisfy criterion 1, which is an **ordering** claim: *the configuration timestamp precedes the deploy that created the first Durable Object*. That deploy happened at `2026-08-27T16:00:29Z` and no later configuration can precede it. The criterion is about a sequence in the past and stays permanently unsatisfiable; the row is not reopened, because reopening it on a fact it does not assert is exactly the widening the ledger's `Refuted` status was declared to prevent. **The protection is real and the criterion is still refuted — both, without one being used to soften the other.** Owner-stated 2026-08-30 rather than measured here: verifying it needs the Cloudflare account, which this repository does not reach. | — Standing |
