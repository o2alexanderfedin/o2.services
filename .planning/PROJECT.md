...
| Browser peers are full peers, not a lower tier | Owner decision 2026-07-26. The only difference is that a browser cannot bind a listening socket; a relay reservation makes it dialable, proven cross-device in Phase 3. Client-mode-only DHT and leaves-only reduce were inherited assumptions, now reversed. Background-tab throttling is a lease-duration problem, not a capability one | — Done |
