...
| AUTH-05 | Phase 19 — Quorum Composition & Owner-Domain Attestation | **Built, not wired** — corrected 2026-08-02 by 19-01, which gave the per-owner grouping its first production caller: `discoverCandidates` fills `CandidateSet.replicaSets` from the certificates it just qualified, against the clock that qualified them, and `NodeDescriptor` now carries each node's certificate instead of discarding everything but its user key. **What is still open is the consuming half.** Nothing reads `replicaSets`, no entry point puts a user key into `PlacementRequest.ownerId` (Plan 19-09 unifies `--owner-id` with the user key), and attestationReceipt has no production caller — so an owner's replicas are grouped and the agreement over them is never labelled |
