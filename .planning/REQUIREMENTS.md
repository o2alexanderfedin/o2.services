...
milestone audit. Of the 27 now unchecked, **23 moved** and **4 were never checked**;
none of the 23 moved because the work was undone — with one exception, VER-02, whose
...
**The 27 unchecked boxes are 1 + 25 + 1**, and the three markers are not one label:
1 are *Built, not wired*, 25 are *Partial*, and VER-02 alone is *Not started*.
...
assertion rather than a copy of one.) **`MR-02` is the last row in the first bucket**,
where until 2026-08-05 there were seven — the six `CHURN-0*` rows left it that day, four
of them by being delivered and two by being re-categorised as *Partial* with the leg that
is actually open named. A bucket of one is worth watching: the marker's whole purpose is
to name mechanisms nothing calls, and the milestone exists to empty it.
...
deferred v2 idea — it is being delivered now, one requirement per named obligation, and
the decoder that satisfies six of the seven below has no production caller this phase
by design (out of this phase's named scope; only its bundle cost is measured, in Plan
25-03).
...
- [ ] **EGR-01**: An egress reading distinguishes *the run registered no sovereign data*
...
**The one `[ ]` is the phase's own residue.** `CRYPTO-03` ledgers the certificate-lifecycle
facades, which are built, tested and merged and which nothing calls — the *built, not wired,
and not counted* shape this milestone exists to eliminate, produced by the milestone's own
work. It is marked under the entry-point-reachability convention at the top of this document
rather than by how much work it took, and this phase deliberately does not improve it by
wiring anything: see the row for the two owner non-decisions and their priced cost.
...
- [ ] **CRYPTO-03**: The certificate-lifecycle facades are ledgered under the entry-point-reachability convention rather than existing as real-but-uncounted code
...
| BROW-02 | Phase 20 — Single Job Path, Ledger & Churn Resilience | **Partial** — the wire landed in Plan 20-02 and the reading did not. What is measured: both node factories build a real `StartOutcomeLedger` and **record their own start row into it at construction**, which is the half that is easy to omit and fatal to omit — `serveAgent` records only what a peer told it, so without the own row two tabs hand each other back their own rows and `mergeOverlapping` pins every merged report at 1 forever. `L1` is the entry that pins it and `L2` pins the hook itself; `L3` pins the wire answer, whose observed failure is a merged report that no longer carries a browser family the asking node has no expression to produce. Read over an in-process fabric by `packages/net/src/start-report.test.ts`. The demo's own deferral of this — behind `MAX_REPORTED_COUNT` and `isStartBrowserLabel` — is spent, both bounds are enforced in `parseCounts`, and `L4` and `L5` are what stop either being loosened. **Two things are open and neither is a wiring step.** The reading: no tab has yet been shown displaying counts it could only have learned from a peer, which was Plan 20-06's and which that plan did not deliver — it recorded no plant, no span and no exit code for the `e2e` project, and the mutation-ledger entry its own note asks for is deliberately absent because the observed text was never captured. And the **row-count bound**, which 20-02 measured rather than closed: `parseCounts` caps each entry's magnitude and validates each label but not the array length, and NET-08's 8 MiB frame ceiling implies a crossover at **191 099 rows** at 43.90 encoded bytes per row — 10 000 well-formed rows merged with nothing refusing any of them. That is not a bound on a quantity that grows by one row per distinct `(browser, result)` a node is ever told about |
...
| SCHED-06 | Phase 13.1 — Node-Side Admission & Transport Bounds | **Done** — a node at its slot limit refuses `exec` with a named reason rather than queueing. `CountingExecutor` is composed **outermost** in both factories so the count is of tasks actually admitted, and `LocalCapacity` answers the offer; `serveAgent({capacity: admission})` is the wire surface. Verified in Phases 13.1, 14, 16, 18 and 23 — the row was absent from this table until 2026-08-08 while the mechanism was wired the whole time |
...
| NET-09 | Phase 13.1 — Node-Side Admission & Transport Bounds | **Partial** — re-dispatch on a failed send is wired inside `submitJob`'s generation loop and is what the requirement turns on. The `'sender'` classification that distinguishes *this node refused* from *that node failed* is reachable only through `remoteDispatch`, and `remoteDispatch` has no production caller; `packages/net/src/churn.ts` states the deferral in its own words and `remoteDispatch` is disposed under `deferred-in-source` in `reachability-dispositions.ts`. Row added 2026-08-08 |
...
| X509-01 | Phase 25 — X.509 Certificate Profile | **Built, not wired** — `id-Ed25519` (`1.3.101.112`) is the only OID `checkAlgorithm` accepts, checked at all three `AlgorithmIdentifier` positions (`TBSCertificate.signature`, `SubjectPublicKeyInfo.algorithm`, `Certificate.signatureAlgorithm`); every other OID refuses `unrecognised-algorithm` by construction, not by a deny-list. Proved by `x509.test.ts`'s golden-fixture acceptance test (Ed25519 decodes correctly) and the four named-ban tests below. `decodeX509Certificate` has no production caller — this phase does not wire the decoder into enrollment, issuance, or the demo (out of this phase's named scope; only its bundle cost is measured, in Plan 25-03) |
| X509-02 | Phase 25 — X.509 Certificate Profile | **Built, not wired** — SHA-1 (`sha1WithRSAEncryption`), P-192 and P-224 (both via `id-ecPublicKey` with the curve OID in `parameters` — the outer OID alone refuses, so two distinct planted fixtures reach the identical `unrecognised-algorithm` path, satisfying "each with its own test") and RSA (`rsaEncryption`) are each refused by name, in `x509.test.ts`'s `describe('algorithm allow-list (X509-01/02)', ...)` block, four separate tests, each asserting the specific OID in `result.failure.oid`. `decodeX509Certificate` has no production caller — same deliberate non-wiring as X509-01 |
| X509-03 | Phase 25 — X.509 Certificate Profile | **Built, not wired** — `checkDerCanonical` (Plan 25-01) is wired as `decodeX509Certificate`'s final gate, after every structural/algorithm/extension check, so a semantic refusal still fires specifically rather than every malformed case collapsing into one generic reason. Proved at whole-certificate scope (not just the standalone-TLV scope Plan 25-01 proved) by three planted mutations in `x509.test.ts`'s `describe('certificate-level DER canonicalisation gate (X509-03)', ...)`: a non-minimal `signatureValue` length, a redundant leading `0x00` on `serialNumber`, and unzeroed `BIT STRING` padding bits on `subjectPublicKey`, each refusing `non-canonical-encoding`. `decodeX509Certificate` has no production caller — same deliberate non-wiring as X509-01 |
| X509-04 | Phase 25 — X.509 Certificate Profile | **Built, not wired** — `MAX_CERTIFICATE_BYTES` (Plan 25-01) is checked by `preParseCheck` before `decodeDer` runs at all, proved by `x509.test.ts`'s `describe('MAX_CERTIFICATE_BYTES is a bound, not a ban (X509-04)', ...)` (at-the-bound accepted, one-past refused by name) and the ordering proof in `describe('certificate-size gate runs before any TLV parsing (X509-04)', ...)` (a `decodeDer` call-count assertion of zero). `decodeX509Certificate` has no production caller — same deliberate non-wiring as X509-01 |
...
| AOTW-06 | Phase 26 — elfconv Compiled to Wasm | **Not started** — and it is a measured negative, reported unmet rather than reworded, on the precedent AOT-05 already carries. The gate returned **NO-GO** (`26-GATE.md`): 21 of 27 non-test translation units do not compile for `wasm32-wasi`, and the cause is **glog**, which branches on Windows, Cygwin, Linux, Android, macOS, the BSDs and Emscripten and has **no `__wasi__` branch at all** — the `wasm32-wasi-threads` arm moves the refusal from `glog/logging.h:51` to `glog/platform.h:58` and removes nothing. No `elflift.wasm` was linked, so no bitcode was produced and no `sha256` comparison against native elfconv was possible. The symbol half of the gate is separately an **upper bound and conclusive in neither direction**, since `wasm-ld` pulls only the members it needs and the whole process and signal residue lives in `Program.cpp.o`, `Signals.cpp.o`, `CrashRecoveryContext.cpp.o`, `Process.cpp.o`, `LockFileManager.cpp.o` and `Watchdog.cpp.o` of LLVMSupport. **What blocks this id is a cross-build nobody has paid**: precondition 3 of `26-CONTEXT.md` records that the only cross-compiled LLVM on this host is Emscripten-targeted and cannot be guest code under this project's preview1 sandbox, so a wasm32-wasi LLVM must be built from source before the deliverable is reachable at all |
| EGR-01 | Phase 27 — The Demo UI, Driven by the Real Fabric | **Not started** — measured three times, deferred three times, and minted here so the fourth time is a decision rather than a fourth log line. `egressLines` in `packages/browser/demo/render.ts` emits `0 withheld — and this run registered no sovereign data, so that is the guard reporting it had nothing to hold back, not a proof of sovereignty.` Plan 27-07 read that off the screen after a bring-your-own dispatch that **submitted six owner-pinned shards**, measured why the guard saw nothing (the shards were unplaceable, and the placed arm was refused at authorization before the canonical bytes crossed), and left the copy alone. Plan 27-08 found the same sentence has a fifth renderer. **The change, corrected 2026-08-10 against the source rather than carried forward from the deferred note.** That note said to count *where the guard registers a sovereign CID*, and that is the wrong place: `EgressGuard.guard()` appends no `EgressEntry`, `submitJobWithEgress` gives every hold back in a `finally`, and `sliceManifest` recomputes the manifest from `entries` alone — so a figure read off the guard after a job is either zero or a session-lifetime total, and the tab reads a **sliced** manifest. The count has to be taken in `submitJobWithEgress`, which is the only code that knows the sovereign shard set, and carried through `sliceManifest` as a per-job delta the way `totalBytes` already is. **Five files in one commit:** `packages/net/src/egress.ts` (the field on `EgressManifest`), `packages/net/src/submit-with-egress.ts` (the count, and its delta in `sliceManifest`), `packages/browser/demo/render.ts` (a third arm in `egressLines`), `packages/node/src/demo-region-properties.ts` (`EGRESS_SENTENCES`, which asserts the present words exactly), and UI-SPEC section 5.2, whose copy is fixed verbatim and cannot be amended from inside a surface. **Four specs re-read in the same pass:** `packages/node/src/demo-byo.e2e.test.ts`, `packages/node/src/demo-pi.e2e.test.ts`, `packages/node/src/demo-fabric.e2e.test.ts`, `packages/node/src/demo-liveness.e2e.test.ts`. Decider: **owner** — it adds a field to a type in `@o2/net` and rewrites the copywriting contract's fixed wording for the one sentence that carries the sovereignty claim |
...
| CRYPTO-03 | Phase 28 — One Cryptographic Implementation | **Built, not wired** — the certificate-lifecycle facades now have the ledger row they have never had, and the marker is the honest one rather than a judgement about effort. `packages/core/src/cert-lifecycle.ts` declares four facades — `Subject`, `Issuer`, `Verifier` and `Directory` against `SubjectFacade`, `IssuerFacade`, `VerifierFacade` and `DirectoryPort` — carrying the 19 flows ported from the retired `cert-lifecycle.facade.test.ts` design probe, plus `signCertificate`, `deriveKeySeeds` and the sealed-blob encoding. They are covered on two tiers and the counts were re-measured 2026-08-10 rather than quoted: **28 cases** under `--project node` and **114 across 6 files** under `--project browser`, both exit 0. That 114 is the same 38 cases counted on two tiers and not two independent suites — `cert-lifecycle.test.ts` carries no `.node.` infix so its 28 also run on chromium, firefox and webkit, and `cert-lifecycle.browser.test.ts` adds 10 more per engine, giving `(28 + 10) x 3 = 114`. **Nothing calls any of it.** `createSubject` has no production caller, `createIssuer` has no production caller, and `createVerifier` has no production caller — re-derived 2026-08-10 against the tree with this file's own comment-stripped call-site instrument, not asserted in prose. The module is imported by exactly one file in the repository, its own `cert-lifecycle.test.ts`; after Plan 28-01 moved the backend block out, even `cert-lifecycle.browser.test.ts` imports `ed25519-backend.ts` instead. **The non-wiring is deliberate and this phase does not take the decision by side effect.** The facades are absent from `packages/core/src/index.ts` on an owner non-decision priced at **12 callable barrel exports**, which against the readings measured after Plan 28-01 would move the unreachable ceiling 73 to 85 and `OPEN_FINDING_CEILING` 47 to 59. That price is a projection from the export list rather than a reading taken with them exported, and it is marked so. The comment at `index.ts:401` still quotes the **pre-merge** pair 75 to 87 and 49 to 61 and is stale by the same drift this plan corrected in the WIRE-02 row above; it is recorded here rather than edited, being outside this plan's file set. Wiring the port into `verifyChain` or `verifyCertificate` is the separate and larger owner ruling, because it is a fail-open versus fail-closed decision on a trust path and the `verifiedPeers` staleness window needs a sited constant first |
