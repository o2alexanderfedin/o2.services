...
| MR-02 | Phase 16 — Decomposable Tree-Reduce Wiring | **Unmeasured.** Phase 16 ran the aggregation over public shards whose inputs travel to the executors by CID, so no partial was computed over an owner's own data and nothing distinguished a map that moved data from one that did not. Needs a sovereign map with an egress manifest, not a wiring step |
| MR-03 | Phase 16 — Decomposable Tree-Reduce Wiring | Wired at `bin/agent.ts` and `bin/bench.ts` via `reduceJob` (Phase 16). The demo still merges with a linear scan — `answerOf` in `packages/demo/src/job.ts`, called from `packages/browser/demo/main.ts`. That half is WIRE-02, Phase 22 |
| MR-04 | Phase 16 — Decomposable Tree-Reduce Wiring | Wired at `bin/agent.ts` and `bin/bench.ts` via `reduceJob` (Phase 16). The demo still merges with a linear scan — `answerOf` in `packages/demo/src/job.ts`, called from `packages/browser/demo/main.ts`. That half is WIRE-02, Phase 22 |
| MR-05 | Phase 16 — Decomposable Tree-Reduce Wiring | Wired at `bin/agent.ts` and `bin/bench.ts` via `reduceJob` (Phase 16). The demo still merges with a linear scan — `answerOf` in `packages/demo/src/job.ts`, called from `packages/browser/demo/main.ts`. That half is WIRE-02, Phase 22 |
| MR-06 | Phase 16 — Decomposable Tree-Reduce Wiring | Wired at `bin/agent.ts` and `bin/bench.ts` via `reduceJob` (Phase 16). The demo still merges with a linear scan — `answerOf` in `packages/demo/src/job.ts`, called from `packages/browser/demo/main.ts`. That half is WIRE-02, Phase 22 |
| MR-07 | Phase 16 — Decomposable Tree-Reduce Wiring | Wired at `bin/agent.ts` and `bin/bench.ts` via `reduceJob` (Phase 16). The demo still merges with a linear scan — `answerOf` in `packages/demo/src/job.ts`, called from `packages/browser/demo/main.ts`. That half is WIRE-02, Phase 22 |
