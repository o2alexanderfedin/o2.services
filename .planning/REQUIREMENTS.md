...
| VER-08 | Phase 19 — Quorum Composition & Owner-Domain Attestation | **Built, not wired** — reason corrected 2026-08-03 by 19-06, which gave the receipt its first production caller: `submitJob` attaches one to every shard and every job, derived from each agreeing replica's signature over that shard's task and result rather than from the node ids the requestor chose. Two halves stay open. **Nothing signs yet**: `attestResults` has no production caller, so a receipt taken from a node in this repository today reads the named absence rather than a strength, and Plan 19-15 composes that wrapper at the two factories. And the redundancy clause itself — a sovereign task executing across an owner's own multi-node set — is reached by no entry point, because none labels a shard `sovereign` against a real owner's node set; 19-09 measures it |
| VER-09 | Phase 19 — Quorum Composition & Owner-Domain Attestation | **Built, not wired** — reason corrected 2026-08-03 by 19-06. One verified replica reads `owner-attested` on `ShardResult`, from the same expression that reads `owner-domain` at two and `independent` at two operators, so the one-node case is not special-cased anywhere. Open on the same two halves as VER-08: `attestResults` has no production caller, so nothing has yet produced the signature that reading is derived from (19-15), and no surface renders the receipt — the CLI is 19-10 and the demo UI is 19-11 |
| VER-10 | Phase 19 — Quorum Composition & Owner-Domain Attestation | **Built, not wired** — reason corrected 2026-08-03 by 19-06. `classifyAttestation` is still the only place the three labels are computed, and `submitJob` now reports its answer per shard and, weakest-first, per job — so the stronger claim cannot be reported for a one-operator agreement. Open: `attestResults` has no production caller, so the distinction is established today only in specs that sign their own fixtures (19-15); and no display site carries it — 19-10 and 19-11 |
...
| AUTH-05 | Phase 19 — Quorum Composition & Owner-Domain Attestation | **Built, not wired** — corrected 2026-08-02 by 19-01, which gave the per-owner grouping its first production caller: `discoverCandidates` fills `CandidateSet.replicaSets` from the certificates it just qualified, against the clock that qualified them, and `NodeDescriptor` now carries each node's certificate instead of discarding everything but its user key. **What is still open is the consuming half.** Nothing reads `replicaSets`, and no entry point puts a user key into `PlacementRequest.ownerId` (Plan 19-09 unifies `--owner-id` with the user key). 19-06 gave the labelling half a production caller — every shard now carries a receipt — but `attestResults` has no production caller, so nothing in this repository signs the result that receipt would be derived from: an owner's replicas are grouped, and the agreement over them is labelled only in specs that sign their own fixtures |
