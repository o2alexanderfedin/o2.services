...
| SCHED-01 | Phase 18 — Discovery, Capacity & Placement | **Built, not wired** — corrected 2026-08-02, and the correction is the interesting part. This row claimed `discoverExecutors` was uncalled; plan 18-05 gave it its first production caller (`packages/net/src/discover-candidates.ts:152`) and `requirements-ledger.node.test.ts` caught the row the moment the phase branch met `develop`. **The gap moved up one level rather than closing**: `discoverCandidates` has no caller outside tests, so the bridge exists and nothing crosses it. Plan 18-06 wires it to `bin/bench.ts --discover`, and this row is expected to fail again on that day — which is the point of writing the claim at the frontier instead of at the symbol |
