...
**The 33 unchecked boxes are 14 + 18 + 1**, and the three markers are not one label:
14 are *Built, not wired*, 18 are *Partial*, and VER-02 alone is *Not started*.
...
| VER-03 | Phase 19 — Quorum Composition & Owner-Domain Attestation | **Built, not wired** — reason corrected 2026-08-03 by 19-06, which gave the composer its first production caller: `composeQuorum` is reachable only through `submitJob`, and a public shard at redundancy two or more is placed on the member set it returns, so the shared-dependency rule now decides a placement instead of sitting beside one. The requirement's own wording was reworded the same day by owner ruling; 19-02's implementation of the old *backbone-anchored* phrasing was retracted in `0314208` for keying on node kind. Two halves stay open. **No default path reaches the gate**: composition is attempted only when this requestor holds a certificate for every candidate, and every production submitter except `bin/bench.ts --discover` — off by default — builds its descriptors through `publicNodes`, which states it carries none. And the **durability** half the ruling named — at least one replica of a result pinned somewhere durable — is deliberately unimplemented, because no field on a certificate says whether a node pins durably and a composer that runs before execution cannot ask |
| VER-04 | Phase 19 — Quorum Composition & Owner-Domain Attestation | **Built, not wired** — reason corrected 2026-08-03 by 19-06. `composeQuorum` is reachable only through `submitJob`, so one operator can no longer supply a whole quorum on the path a job actually runs: the composer holds one certificate per operator by construction, and its members are the pool placement is handed for that shard — applied **before** the load preference rather than checked after it, which is the distinction NET-08 and 16-06 were each corrected for. A candidate set too concentrated to compose **degrades** by default, running at whatever redundancy is available, marked degraded, labelled with the strength it established and carrying the composer's own reason; it is refused only when the caller set `onQuorumShortfall: 'refuses-the-shard'`. Open on VER-03's first condition — no production submitter supplies certificated descriptors by default, so nothing reaches the gate outside a spec — and there is no across-process reading yet, which is 19-08's |
| VER-05 | Phase 1 — Portable Kernel & Loopback Map Slice | Done |
| VER-06 | Phase 1 — Portable Kernel & Loopback Map Slice | Done |
| VER-08 | Phase 19 — Quorum Composition & Owner-Domain Attestation | **Built, not wired** — reason corrected 2026-08-03 by 19-06, which gave the receipt its first production caller: `submitJob` attaches one to every shard and every job, derived from each agreeing replica's signature over that shard's task and result rather than from the node ids the requestor chose. **The signing half closed 2026-08-03 by 19-15**: both production factories now compose the signing wrapper outermost around their executor and hand the same identity to `serveAgent`'s signing hook, so a receipt taken from a node in this repository is derived from real signatures rather than from the named absence — measured across real `bin/agent.ts` processes in `packages/node/src/result-signature.node.test.ts`, verified against nothing but the provider's public key. What stays open is the redundancy clause itself — a sovereign task executing across an owner's own multi-node set is reached by no entry point, because none labels a shard `sovereign` against a real owner's node set; 19-09 measures it, and that is a claim about an argument value rather than about a caller |
| VER-09 | Phase 19 — Quorum Composition & Owner-Domain Attestation | **Built, not wired** — reason corrected 2026-08-03 by 19-06. One verified replica reads `owner-attested` on `ShardResult`, from the same expression that reads `owner-domain` at two and `independent` at two operators, so the one-node case is not special-cased anywhere. The signing half closed 2026-08-03 by 19-15 — both factories compose the wrapper and the readings come from real signatures now. What stays open is the display half: `describeAttestation` has no production caller, so the three labels are rendered for a human nowhere, and a receipt nothing displays is the *built, not wired* condition in a new place. The CLI is 19-10 and the demo UI is 19-11 |
| VER-10 | Phase 19 — Quorum Composition & Owner-Domain Attestation | **Built, not wired** — reason corrected 2026-08-03 by 19-06. `classifyAttestation` is still the only place the three labels are computed, and `submitJob` now reports its answer per shard and, weakest-first, per job — so the stronger claim cannot be reported for a one-operator agreement. The distinction is no longer established only in specs that sign their own fixtures: 19-15 composed the signing wrapper at both factories on 2026-08-03, so the labels are computed from signatures real nodes produced. Open: no display site carries it — `describeAttestation` has no production caller, which is that absence stated as a call-site fact rather than as a plan reference. 19-10 and 19-11 |
...
| AUTH-04 | Phase 17 — Node Identity & Enrollment | **Partial** — corrected 2026-08-01; this row claimed no production caller and both factories had one. **Provider-gating and rate-limiting are wired and measured**: `new EnrollmentAuthority` at `fabric-node.ts:1313` / `browser-node.ts:924`, reachable from `bin/agent.ts --issues-certificates`, with the provider signing key generated on-device into a file separate from `.identity.key` so `issuerKey !== nodeKey` always holds; a burst through the production `enrol` request path is refused past the stated threshold (`enrollment.node.test.ts`, criterion 3). **The *per-user* limiter buys no cost, measured** — it keys on `userKey` and a fresh user key is one `ed25519.keygen()` call, so twenty requests under twenty distinct keys all succeed against it, and no deletion turns that assertion red. **19-05 added the bound that does answer the cost clause**: `maxIssuedPerWindow`, an aggregate on how many certificates one provider signs per window whoever asked — a quantity no request field can rotate around — plus `IssuanceLedger`, a host-supplied port both budgets read so the authority holds no history of its own. Both are required unions with named sentinels. The same twenty free keygens are refused past the stated number and admitted in full against `'issues-without-an-aggregate-budget'`, side by side (`enrollment.test.ts`). **What remains open, and why the marker has not moved:** both production factories write the sentinels, so `fabric-node.ts:1313` / `browser-node.ts:924` still have no aggregate budget and no durable history, and the threshold is still per provider *uptime* on that configuration. Plan 19-07 supplies each tier's durable ledger and takes the cross-process reading across a real restart, which is the load-bearing one. Also recorded rather than mitigated: the aggregate budget lets anyone who can dial a provider consume its whole window at one keygen per attempt, `serveAgent` serving enrolment unauthenticated; the per-verifier answer to that is **untested here**, because every fixture and the demo are single-provider |
| AUTH-05 | Phase 19 — Quorum Composition & Owner-Domain Attestation | **Built, not wired** — corrected 2026-08-02 by 19-01, which gave the per-owner grouping its first production caller: `discoverCandidates` fills `CandidateSet.replicaSets` from the certificates it just qualified, against the clock that qualified them, and `NodeDescriptor` now carries each node's certificate instead of discarding everything but its user key. **What is still open is the consuming half.** Nothing reads `replicaSets`, and no entry point puts a user key into `PlacementRequest.ownerId` (Plan 19-09 unifies `--owner-id` with the user key). 19-06 gave the labelling half a production caller — every shard now carries a receipt — and 19-15 closed the signing half on 2026-08-03, so the agreement over an owner's replicas is now labelled from signatures real nodes produced rather than from fixtures a spec signed for itself. What remains is the consuming half named above, and it is a claim about an argument value rather than about a caller: no entry point puts a user key into `PlacementRequest.ownerId` |
