...
- [x] **AOTW-04**: Every non-test translation unit of this repository's elfconv fork is compiled for `wasm32-wasi`, and each result carries its own error text and its produced object size; no exit code stands in for a produced object
...
`capability.ts:219`, `enrollment.ts:702`, `:740`, `:759`, `:874` and `discovery.ts:122` — and
routes through neither. So these rows record a **duplication removed from a package and a
