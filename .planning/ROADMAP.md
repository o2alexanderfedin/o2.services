...
**This is a defect on the hosted tier and NOT the same path `HOST-03` closed.** That requirement is about `/o2/<nodeKey>` VALUE records, measured writer-stopped-then-read on a local workerd. Provider records go through `@libp2p/kad-dht`'s separate providers store under a `/dht/provider/` prefix — which is one of the two namespaces `DoDatastore.put` refuses unless a sweep is armed. Criterion 2's clock cannot even start until this is fixed: there is no observable *published at T* to be gone at T + validity. Criterion 2 stays OPEN, and `HOST-13` stays `Partial`, now for a measured reason instead of an unattempted one.
