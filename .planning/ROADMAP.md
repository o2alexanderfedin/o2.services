...
**Status 2026-09-16 — all five criteria met. `independent` is now unreachable on this fabric, which is what the phase was for.**

**What it cost, criterion by criterion.** (1) `composeQuorum` refuses a one-issuer member set with `single-issuer-quorum` under the default rule and composes it when waived; the rule is expressed as **construction** — candidates grouped by issuer and taken round-robin — so a two-provider fabric produces a spread member set rather than merely accepting one. The check is sited **after** the path check, so `shared-relay-dependency` keeps speaking for every set it already spoke for, and a one-operator pool still says `insufficient-operators`. (2) `classifyAttestation` returns `'single-issuer'` for many operators and one issuer, `'independent'` only when **both** dimensions exceed one; two providers over one user key is one operator and is correctly `owner-domain`. (3) The ordering holds across the insertion, and `packages/node/src/attestation-claims.node.test.ts` now keeps the no-string-comparison property over 217 non-test files with three positive controls, three negative controls, one exemption scoped to two **function names** rather than to lines, and a per-function dead-exemption check. (4) **Eleven plants across the phase, eleven reds, no green plant** — each restored by the surgical inverse and `cmp`-verified; `quorum.ts` ends the phase byte-identical to what 45-01 committed. (5) Every surface that names a strength now names how many providers stood behind it, from one sentence in `describeAttestation`.

**The live path waives the refusal and keeps the preference, and that is the phase's own decision, recorded at `.planning/OWNER-ACTIONS.md` §3c.** With one provider a defaulted refusal would stop every public shard composing — redundant verification dying, which the owner's ruling did not buy. The waiver turns off the **refusal**, not the **construction**, so the sentence above this block stays true: a second provider makes `independent` reachable with no code change. **A case pins it** — a two-issuer pool composed with the literal `requireDistinctIssuers: false`, asserting the members spread across both issuer groups. It was worth writing: a plant nesting the round-robin inside the flag reddened **one case out of thirty-two** while `tsc` stayed clean.

**Two false greens were caught, and neither was caught by a plant.** (a) When the surface format changed, `bench-attestation`'s parser regex stopped matching anything the driver emits — and the neighbouring **absence** check, whose only instrument is that regex, went on passing **vacuously on every rung**. It arose in the ordinary course of editing a producer. (b) `quorum.test.ts` read its description assertions **by array index**; inserting the fourth strength would have retargeted `descriptions[2]` onto a sentence containing the same substring, staying green while leaving `independent` unasserted. Both are written down beside the code they concern.

**The census: 14 cases in `quorum.test.ts` plus 13 spec files.** Four were given a second authority, **nine were deliberately waived** because a second issuer would have moved the orderings they actually assert, two moved to `single-issuer`, and one was found **inverted** — a Phase 44 case asserting *"the strength is unchanged across the pair"*, true while the dimension was inert and false the moment it bound. One reading was **added rather than moved**, because moving it would have satisfied the plan and cost the case its argument.

**What this phase does NOT fix**, unchanged from the text above: an attacker who reaches two providers; providers colluding or one party running several, which makes **issuer diversity a PROXY for party diversity**; the cost of an identity; and `relayIds`, the same shape of defect, not examined.

Plans:
- [x] `45-01-PLAN.md` — the mechanism: the `single-issuer` strength, `requireDistinctIssuers` defaulting true, the `single-issuer-quorum` refusal, issuer-grouped round-robin composition, the live path's explicit waiver, and the cases carrying criteria 1 and 2 (wave 1)
...
**Plans**: `46-CONTEXT.md`, then **6 plans in 5 waves**. `46-01-PLAN.md` (naming.ts — the field, and every naming.ts codec that must agree on it, wave 1), `46-02-PLAN.md` (protocol.ts's wire codec and its round-trip case, wave 2, parallel with 03), `46-03-PLAN.md` (the guardNetworkReach adapter and its own unit tests, wave 2, parallel with 02), `46-04-PLAN.md` (two mutation-ledger entries — the codec-drop plant and criterion 5's sovereign-check plant — wave 3, **alone**), `46-05-PLAN.md` (wiring both node factories, and proving it on both tiers: the requestor's view over real RPC on the Node tier, the operator's view through a real BrowserNode on the browser tier, wave 4), `46-06-PLAN.md` (the full node lane in one sweep, and the criteria-to-test map, wave 5).
