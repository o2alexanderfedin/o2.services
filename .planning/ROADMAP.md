...
**Success Criteria** (what must be TRUE):
  ...
  3. The libp2p keychain on both tiers that use it derives a real DEK — **`pass` AND `dek.salt`, both**, since either alone leaves the empty string — and a test asserts the derived DEK is not `''` rather than asserting the options object was passed, because the options object was always passable and the defect is that it did nothing
